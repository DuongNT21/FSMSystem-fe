      ...
      <div className="mb-10">
        <nav className="flex text-sm font-medium text-slate-500 mb-4">
          <ol className="flex items-center space-x-2">
            <li><Link className="hover:text-rose-500 transition-colors" to="/">Trang chủ</Link></li>
        ...
        <h2 className="text-4xl font-extrabold text-slate-900">Sản Phẩm Bó Hoa</h2>
        <p className="text-slate-500 mt-2 max-w-2xl">Khám phá thế giới hoa tươi nghệ thuật với những thiết kế độc bản từ FlowerShop.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="lg:w-72 shrink-0 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold uppercase tracking-widest text-slate-900 mb-4">Tìm kiếm sản phẩm</h3>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div className="space-y-8">
              <div>
                <p className="text-[13px] font-bold text-slate-800 mb-4 uppercase tracking-wide">Vật tư (Hoa & Phụ kiện)</p>
                <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {materials.map(m => (
                    <label key={m.id} className="flex items-center gap-3 cursor-pointer group">
                      <input 
                        ...
                        onChange={() => setFilters({...filters, materialId: m.id.toString()})}
                      />
                      <span className={`text-sm transition-colors ${filters.materialId === m.id.toString() ? 'text-rose-500 font-bold' : 'text-slate-600 group-hover:text-rose-500'}`}>
                        {m.name}
                      </span>
                    </label>
                  ))}
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input 
                      ...
                      onChange={() => setFilters({...filters, materialId: ""})}
                    />
                    <span className={`text-sm transition-colors ${filters.materialId === "" ? 'text-rose-500 font-bold' : 'text-slate-600 group-hover:text-rose-500'}`}>
                      Tất cả
                    </span>
                  </label>
                </div>
              ...
              <div>
                <p className="text-[13px] font-bold text-slate-800 mb-4 uppercase tracking-wide">Khoảng giá (VNĐ)</p>
                <div className="px-1">
                  <input 
                    className="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-rose-500" 
                    max="5000000" min="0" step="100000" 
                    type="range"
                  ...
                  <div className="flex justify-between items-center mt-4 text-[11px] font-extrabold text-slate-500 uppercase">
                    ...
                    <span className="bg-slate-50 px-2 py-1 rounded">{filters.maxPrice.toLocaleString()}đ</span>
                  </div>
                </div>
              </div>
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center justify-between mb-8 pb-4 border-b border-slate-100 gap-4">
            <p className="text-sm text-slate-500 font-medium">Hiển thị <span className="text-slate-900 font-bold">{bouquets.length}</span> sản phẩm</p>
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 font-bold uppercase tracking-wider shrink-0">Sắp xếp:</span>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="animate-pulse bg-slate-100 rounded-3xl aspect-[4/5]"></div>
              ))}
            </div>
          ) : bouquets.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-slate-400">Không tìm thấy sản phẩm nào phù hợp.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
              {bouquets.map((b) => {
                return (
                  <div key={b.id} className="group bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-rose-500/5 transition-all duration-500">
                    <Link to={`/shop/${b.id}`} className="block relative aspect-[4/5] overflow-hidden">
                      <img 
                        alt={b.name} 
                        className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" 
                        src={b.images?.[0]?.image || "https://picsum.photos/400/500?flower"} 
                      />
                    ...
                    <div className="p-6">
                      <p className="text-[10px] font-bold text-rose-500 uppercase tracking-[0.2em] mb-2">Bó Hoa Tươi</p>
                      <Link to={`/shop/${b.id}`}>
                        <h4 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-rose-500 transition-colors line-clamp-1">{b.name}</h4>
                      </Link>
                      <p className="text-sm text-slate-500 line-clamp-2 mb-5 leading-relaxed">{b.description}</p>
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          <p className="text-xl font-black text-slate-900 tracking-tight">{b.price.toLocaleString()}đ</p>
                        </div>
