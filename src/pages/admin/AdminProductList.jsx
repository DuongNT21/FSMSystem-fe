...
export const AdminProductList = () => {
  ...
  try {
    ...
    setBouquets(
      response?.data?.map(item => item.bouquet) ?? []
    );
    ...
    <div className="p-8">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 mb-8 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Khoảng giá (VND)</label>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                    ...
                    <td className="px-6 py-4">
                      <div className="w-16 h-16 rounded-lg overflow-hidden border border-slate-200">
                        <img 
                          ...
                          src={b.images?.[0]?.image || "https://picsum.photos/200/200?flower"} 
                        />
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {b.bouquetsMaterials?.length > 2 && (
                          <span className="text-[10px] text-slate-400">+{b.bouquetsMaterials.length - 2}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${b.status === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          <p className="text-sm text-slate-500">Hiển thị {bouquets.length} của {total} sản phẩm</p>
          <div className="flex gap-2">
            <button 
              disabled={page === 0}
